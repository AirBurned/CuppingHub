...
export default function ScoresPage() {
  ...
  const filledKeys = spiderKeys.filter((k) => evaluations.some((e) => (e[k] as number | null) !== null))
  ...
  return (
    <div className="min-h-screen bg-light">
      ...
      <main className="max-w-4xl mx-auto px-4 py-6">
              ...
              <tbody>
                ...
                <tr className="border-t-2 border-warm-300 bg-warm-50">
                  ...
                  <td className="text-center px-3 py-3 font-bold tabular-nums text-lg text-primary-dark">
                    {(evaluations.reduce((s, e) => s + (e.totalScore ?? 0), 0) / evaluations.length).toFixed(2)}
                  </td>
                </tr>
              </tbody>
            </table>
        ...
        {/* Notes */}
        {evaluations.some((e) => e.descriptors || e.comment || e.aromaGroundComment || e.aromaBrewedComment) && (
          <div className="bg-white rounded-xl border border-warm-200 p-6">
            <h2 className="font-display text-lg font-semibold mb-4">Заметки команды</h2>
            <div className="space-y-3">
              {evaluations.filter((e) => e.descriptors || e.comment || e.aromaGroundComment || e.aromaBrewedComment).map((e, i) => (
